...
func enter() -> void:
	...
	# Clear drop area targets, as we only care about enemy targets to be selected
	...
	# When aiming, we don't care about card drop area so we don't get false targets
	# That's also why we need to turn it back on when we enter the CLICKED state
